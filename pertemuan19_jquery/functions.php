...
<?php  
	...
	$username = htmlspecialchars(strtolower(stripslashes($data ["username"])));//stripcslashes berfungsi untuk membersihkan karakter backslash sedangkan strtolowwer berfungsi supaya yang dimasukkan user itu huruf kecil, misal jika user registrasi username dengan huruf besar,tapi sewaktu login user juga bisa dengan huruf kecil.
	$email = htmlspecialchars($data ["email"]);
	$password = mysqli_real_escape_string($conn, $data ["password"]);
	$kpassword = mysqli_real_escape_string($conn, $data["kpassword"]);//fungsi tersebut untuk memungkinkan user memasukan password yang ada tanda kutip, dan tanda kutip akan dimasukkan kedatabase secara aman. fungsi tersebut butuh 2 parameter.


	//Cek apakah username dan email sudah ada di dalam database atau belum.
	$result= mysqli_query($conn, "SELECT * FROM users WHERE username = '$username' OR email = '$email' ");//ambil data tabel users dengan mysqli_query pada username dan email untuk tujuan agar misal ada username / email sudah terdaftar maka gagal masuk.
	if (mysqli_fetch_assoc($result)){ //cara baca: jika fungsi ini bernilai true(data ssuda ada didalam database), maka berhentikan functionnya
		echo "<script>
				alert('Username / Email sudah terdaftar slur!');
			</script>";
		return false; 
	}
	...
	if ($password !== $kpassword){
		echo "<script>
				alert('Konfimasi password tidak sesuai slur!');
			</script>";
		return false;// supaya langsung masuk ke else (register.php)
	}

	//Enkripsi password
	$password = password_hash($password, PASSWORD_DEFAULT); //enkripsi password menggunakan password_hash(), jangan md5()

	//Jika sudah lolos cek, maka data dimasukkan ke database
