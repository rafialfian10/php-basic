...
<html>
<head>
	<title class="hidden">Data Pemain Cadangan FC</title>
	...
	<style>
		...
		@media print{ /*sintaks css yang berlaku ketika halaman di print*/
			.hidden{
				display: none;
			}
		}
...
<body>
	...
	<h2 style="background-color: red;" class="hidden">SELAMAT DATANG</h2>
	<a href="print.php" target="blank" ><img src="img/print.jpg" class="print"></a><br>
	<a href="insert.php" class="hidden", class="h2">Tambah sedulur Pemain Cadangan FC</a><br><br>
...
<!-- 	Navigasi halaman -->
	...
	<?php if($halamanAktif > 1) : ?>
		<a href="?halaman=<?php echo $halamanAktif - 1 ?>" class="hidden"><button> &laquo </button> </a>
	<?php endif; ?>

	<!-- navigasi angka -->
	<?php for($i = $startNumber; $i <= $endNumber; $i++) : ?>
		<?php if($i == $halamanAktif) : ?><!-- jika $i diisi halaman aktif maka font akan menjadi bold -->
			<a href="?halaman=<?php echo $i;?>" class="hidden" ><button class="navigasi"> <?php echo $i; ?>  </button></a><!-- href diisi data halaman yang dikirimkan. -->
		<?php else : ?><!-- jika selainnya maka font biasa -->
			<a href="?halaman=<?php echo $i; ?>" class="hidden"><button> <?php echo $i; ?></button> </a> 
		<?php endif; ?>
	<?php endfor ; ?>

	<?php if($halamanAktif < $jumlahHalaman) : ?>
		<a href="?halaman=<?php echo $halamanAktif + 1 ?>" class="hidden"><button> &raquo </button> </a> 
	<?php endif; 
?>

	<!-- Form keyword pencarian -->
	<form action="" method="post">
		<input type="text" name="keyword" placeholder="masukan keyword pencarian..." size="30" autofocus="" autocomplete="off" id="keyword" class="hidden">
	...
	<a href="logout.php" onclick ="return confirm('Are you sure?')" class="hidden"><button>Logout</button></a>
